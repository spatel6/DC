...
var quarterChart;// = dc.pieChart('#quarter-chart').on("postRedraw", function(chart){self.ChartChangeEvent();});
var dayOfWeekChart;// = dc.rowChart('#day-of-week-chart').on("postRedraw", function(chart){self.ChartChangeEvent();});
var moveChart;//= dc.lineChart('#monthly-move-chart').on("postRedraw", function(chart){self.ChartChangeEvent();});
var volumeChart;//= dc.barChart('#monthly-volume-chart').on("postRedraw", function(chart){self.ChartChangeEvent();});
var yearlyBubbleChart;// = dc.bubbleChart('#yearly-bubble-chart').on("postRedraw", function(chart){self.ChartChangeEvent();});
...
var ViewModel = function () {
    ...
    self.PageLoad = function () {
        ...
        gainOrLossChart = dc.pieChart('#gain-loss-chart').on("filtered.test", function (chart, filter) { self.SetClickedGraph(chart.anchorName()); self.ChartChangeEvent(); });
        fluctuationChart = dc.barChart('#fluctuation-chart').on("filtered.test", function (chart, filter) { self.SetClickedGraph(chart.anchorName()); self.ChartChangeEvent(); });
        quarterChart = dc.pieChart('#quarter-chart').on("filtered.test", function (chart, filter) { self.SetClickedGraph(chart.anchorName()); self.ChartChangeEvent(); });
        dayOfWeekChart = dc.rowChart('#day-of-week-chart').on("filtered.test", function (chart, filter) { self.SetClickedGraph(chart.anchorName()); self.ChartChangeEvent(); });
        moveChart = dc.lineChart('#monthly-move-chart').on("filtered.test", function (chart, filter) { self.SetClickedGraph(chart.anchorName()); self.ChartChangeEvent(); });
        volumeChart = dc.barChart('#monthly-volume-chart').on("filtered.test", function (chart, filter) { self.SetClickedGraph(chart.anchorName()); self.ChartChangeEvent(); });
        yearlyBubbleChart = dc.bubbleChart('#yearly-bubble-chart').on("filtered.test", function (chart, filter) { self.SetClickedGraph(chart.anchorName()); self.ChartChangeEvent(); });
    ...
    self.SaveGraph = function (data) {
        ...
        setTimeout(function () {
            ...
            var object = {
                ...
                'YearlyBubbleChartImageUri': yearly_bubble_chart_imageUri,
                'GainLossChartImageUri': gain_loss_chart_imageUri,
                'QuarterChartImageUri': quarter_chart_imageUri,
                'DayOfWeekChartImageUri': day_of_week_chart_imageUri,
                'FluctuationChartImageUri': fluctuation_chart_imageUri,
                'MonthlyMoveChartImageUri': monthly_move_chart_imageUri,
                'MonthlyVolumeChartImageUri': monthly_volume_chart_imageUri,
    ...
    self.SaveImage = function (imageName) {
        var canvas;
        switch (self.ClickedGraph()) {
            case 1:
                canvas = $('#yearly-bubble-chart svg')[0];
                break;
            case 2:
                canvas = $('#gain-loss-chart svg')[0];
                break;
            case 3:
                canvas = $('#quarter-chart svg')[0];
                break;
            case 4:
                canvas = $('#day-of-week-chart svg')[0];
                break;
            case 5:
                canvas = $('#fluctuation-chart svg')[0];
                break;
            case 6:
                canvas = $('#monthly-move-chart svg')[0];
                break;
            case 7:
                canvas = $('#monthly-volume-chart svg')[0];
                break;
        }
        //saveSvgAsPng(canvas, 'D:\\Sanjay\\Projects\\DC\\DC\\assets\\img\\' + imageName + '.png');
    ...
    self.ChartChangeEvent = function () {
        ...
        //debugger;
        if (self.isSavedFilterChangeCall()) {
            count++;
            if (count == 7) {
    ...
    self.SaveGraphTest = function () {
        //saveSvgAsPng(document.getElementById("dc-chart"), "diagram.png");

        //$('.load-target').html($('#quarter-chart').html());
